export default {
  ...
  chat: {
    ...
    sandbox: {
      ...
      tabTerminal: '终端',
      tabDesktop: '桌面',
      filesBreadcrumb: '工作区路径',
      filesRefresh: '刷新',
      filesNewDir: '新建文件夹',
      filesNewDirName: '文件夹名称',
      filesUpload: '上传文件',
      filesRename: '重命名',
      filesSave: '保存',
      filesCancel: '取消',
      ...
      filesDeleted: '已删除',
      filesUploaded: '已上传',
      filesOpFailed: '操作失败，请重试',
      filesLoadFailed: '加载失败，请重试',
      filesRetry: '重试',
      filesEmpty: '工作区为空',
      filesUnsupported: '当前沙箱后端不支持文件浏览',
      artifactKindSlides: '演示',
      artifactKindWeb: '网页',
      artifactKindSheet: '表格',
      artifactKindDoc: '文档',
      artifactKindPdf: 'PDF',
      artifactKindImage: '图片',
      artifactKindAudio: '音频',
      artifactKindVideo: '视频',
      artifactKindDiagram: '图表',
  ...
  settings: {
    ...
    sandbox: {
      ...
      workspaceRootNotConfigured: '未配置工作区根目录',
      templateApplied: '已使用',
      ...
      localWorkspaceRoot: '工作区根目录',
      localWorkspaceRootHint: '每个会话的沙箱是该目录下的一个子目录，脚本与文件都落在里面；目录被删除即沙箱被回收。',
      localWorkspaceRootHelp: '必须填写主机上的绝对路径，例如 /data/weknora-local-sandboxes。WeKnora 进程需要对该目录有读写权限。',
      localIsolationRisk: '本地进程后端只有目录边界和 ulimit 资源上限，隔离强度低于容器。对隔离有硬性要求时请改用 Docker 或 Cube。',
      localNetworkHint: '本地子进程直接使用主机网络，出站策略对本后端不生效。',
      localIdleTtl: '空闲回收（秒）',
      localIdleTtlHelp: '沙箱目录多久没有存活进程且无新命令就会被清理，会话继续时重建。留空按 1800 秒。',
      localCpuLimit: 'CPU 时间上限（秒）',
      localCpuLimitHelp: '单条命令可消耗的 CPU 时间（ulimit -t），超时进程组会被整体终止。0 使用内置默认。',
      localMemoryLimit: '内存上限（MB）',
      localMemoryLimitHelp: '单条命令可用的虚拟内存上限（ulimit -v）。0 使用内置默认。',
      ...
      maxConcurrentSandboxes: '同时运行沙箱上限',
      maxConcurrentSandboxesHelp: '该配置在整个工作区内同时保留的沙箱数量上限（跨会话计数）。达到上限后新建沙箱会直接失败，提示先销毁现有沙箱或调高上限。留空或 0 表示不限制；多副本部署时每个副本独立计数，请按副本数折算。',
      envVars: '环境变量',
      envKey: '变量名',
      envValue: '变量值',
      ...
      backendDescriptions: {
        ...
        local: '在 WeKnora 所在主机上为每个会话保留一个工作目录和子进程组，无需容器运行时'
      },
      stepDescriptions: {
