export default {
  ...
  chat: {
    ...
    sandbox: {
      ...
      tabTerminal: 'ターミナル',
      tabDesktop: 'デスクトップ',
      filesBreadcrumb: 'Workspace path',
      filesRefresh: 'Refresh',
      filesNewDir: 'New folder',
      filesNewDirName: 'Folder name',
      ...
      filesOpFailed: 'Operation failed, please retry',
      filesLoadFailed: 'Failed to load, please retry',
      filesRetry: 'Retry',
      filesEmpty: 'The workspace is empty',
      filesUnsupported: 'This sandbox backend does not support the file browser',
  ...
  settings: {
    ...
    sandbox: {
      ...
      workspaceRootNotConfigured: 'Workspace root not configured',
      ...
      localWorkspaceRootHint: 'Each session gets one subdirectory under this root; scripts and files live inside it, and deleting the directory reclaims the sandbox.',
      localWorkspaceRootHelp: 'An absolute host path is required, e.g. /data/weknora-local-sandboxes. The WeKnora process needs read/write access to it.',
      localIsolationRisk: 'The local backend enforces only a directory boundary and ulimit resource caps — weaker isolation than a container. Use Docker or Cube when hard isolation matters.',
      localNetworkHint: 'Local subprocesses use the host network directly; egress policy does not apply to this backend.',
      localIdleTtl: 'Idle reclaim (s)',
      localIdleTtlHelp: 'How long a sandbox directory with no live process and no new command is kept before cleanup. Empty means 1800 seconds.',
      localCpuLimit: 'CPU time cap (s)',
      localCpuLimitHelp: 'CPU seconds a single command may burn (ulimit -t); the whole process group is killed on timeout. 0 uses the built-in default.',
      localMemoryLimit: 'Memory cap (MB)',
      localMemoryLimitHelp: 'Virtual memory a single command may use (ulimit -v). 0 uses the built-in default.',
      ...
      maxConcurrentSandboxes: 'Max concurrent sandboxes',
      maxConcurrentSandboxesHelp: 'How many live sandboxes this config may keep across all sessions in the workspace. Once the cap is reached, new creations fail with a message to destroy a sandbox or raise the limit. Empty or 0 means unlimited; each replica counts independently in multi-replica deployments.',
      envVars: '環境変数',
      envKey: '名前',
      envValue: '値',
      ...
      backendDescriptions: {
        ...
        local: 'Keep a workspace directory and a subprocess group per session on the WeKnora host; no container runtime required'
      },
      stepDescriptions: {
